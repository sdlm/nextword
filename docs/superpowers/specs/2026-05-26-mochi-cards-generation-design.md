...
| Модель выполнения запросов | Message Batches API (async submit + poll) |
...
| Структура команд | Одна блокирующая команда `cards generate` (submit → poll → write), с crash-recovery через state-файл |
...
nextword/cards/
  ...
  client.py    — обёртка Anthropic SDK: submit_batch, poll_status,
                 fetch_results; generate_one (sync, для превью промта)
  pipeline.py  — оркестрация: csv → запросы → submit/poll/collect →
                 cards.json; управление state-файлом
...
Большой статичный блок промта помечается `cache_control: {"type": "ephemeral"}` — он одинаков для всех запросов батча, что даёт экономию 75–90% на кэшируемых входных токенах. (Соответствует требованию навыка `claude-api` о prompt caching.)
...
**Модель:** `claude-sonnet-4-6` (константа в коде, легко заменить на Opus). Батч даёт −50% к цене.

## Поток данных

1. Прочитать `data/export.csv` — пропустить заголовок и пустые строки → список слов.
2. На каждое слово собрать batch-запрос с `custom_id = слово` (маппинг результата обратно).
3. `submit_batch(requests)` → `batch_id`. Записать state в `data/cards_batch.json`:
   `{ "batch_id": ..., "submitted_at": ..., "words": [...], "status": "in_progress" }`.
4. Поллить статус батча до `ended`.
5. Из каждого результата извлечь `tool_use.input` (8 полей) по `custom_id`.
6. Записать `data/cards.json`.

### Формат `data/cards.json`

Ключи в `fields` совпадают с именами полей из `template.md` — чтобы будущий импорт в Mochi был `mochi.create(template_id, fields=card["fields"])`:
...
- **`cards generate`** (блокирующая):
  ...
  - иначе: прочитать csv → собрать запросы → submit → записать state → поллить до конца → записать `cards.json`.
- **`cards preview <word>`** (sync): один запрос `messages.create` с тем же промтом и tool-схемой, печатает JSON одной карточки в stdout. Для отладки качества промта перед массовым батчем. Не использует Batches API и state-файл.

## Обработка ошибок

- **Частичные сбои батча**: результаты со статусом `error`/`expired` пропускаются; успешные пишутся в `cards.json`; в конце печатается список проблемных слов — их можно перегенерировать повторным запуском.
- **State-файл**: предотвращает дубль-сабмит; повторный сбор уже завершённого батча идемпотентен (перезаписывает `cards.json`).
...
- `pipeline.py`: парсинг csv (пропуск заголовка/пустых), сборка запросов с корректным `custom_id`, разбор `tool_use.input` → `fields`, запись `cards.json`, идемпотентность сбора, обработка частичных сбоев — с замоканным `client`.
- `client.py`: корректные параметры запроса (модель, tools, tool_choice, cache_control), разбор результатов — с замоканным Anthropic SDK.
...
`anthropic` и `python-dotenv` уже в `pyproject.toml`. Новых зависимостей не требуется. (`mochi-api-client` остаётся неиспользуемым до задачи импорта.)
