...
- **Частичные сбои батча**: результаты со статусом `error`/`expired` пропускаются; успешные пишутся в `cards.json`; в конце печатается список проблемных слов. Инкрементального ретрая нет: повторный `cards generate` (после `status="collected"`) сабмитит **все** слова из `export.csv` заново и **перезаписывает** `cards.json` целиком. Чтобы перегенерировать только упавшие, временно сократи `export.csv` до нужных слов.
